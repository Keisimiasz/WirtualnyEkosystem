...
Sasiedztwo Srodowisko::ustalSasiedztwo(
    ...
    for (Polozenie p : {P, PG, G, LG, L, LD, D, PD}) {
        ...
        Sasiedztwo::
            zmienIndeksyWgPolozenia(p, wiersz1, kolumna1);
...
void Srodowisko::wykonajSkok(unsigned int wiersz,
    ...
    if (sasiedztwo.ile(PUSTKA) > 0) {
        ...
        Sasiedztwo::zmienIndeksyWgPolozenia(polozenie,
                                            wiersz1, kolumna1);
...
void Srodowisko::wykonajAkcje(unsigned int wiersz,
    ...
    Sasiedztwo::zmienIndeksyWgPolozenia(zamiar.gdzie,
                                        wiersz1, kolumna1);
...
string Srodowisko::doTekstu() const {
    ...
    tekst += "\n Glony * : "
        + to_string(liczba(GLON))
        + "\n Grzyby # : "
        + to_string(liczba(GRZYB))
        + "\nBakterie @ : "
        + to_string(liczba(BAKTERIA))
        + "\n Martwe + : "
        + to_string(liczba(TRUP))
        + '\n';
